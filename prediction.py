def predict_next_month_from_history(model_df, models_dir='models', output_dir='data'):
    ...
    def save_and_visualize_predictions(pred_df, pred_pc, next_month):
    ...
    next_month = determine_next_month(model_df)
    next_df = prepare_next_month_data(model_df, next_month)
    _, next_df = add_moving_averages_per_split(model_df, next_df)
    reg, clf, feats = load_models()
    pred_df, pred_pc = make_predictions(next_df, reg, clf, feats)
    return save_and_visualize_predictions(pred_df, pred_pc, next_month)
